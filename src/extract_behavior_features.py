...
Outputs:
  - outputs/predictions/behavior_embeddings.csv     (sample_id, label, split, status, message + beh_emb_*)
...
def validate_required_columns(df: pd.DataFrame, path: Path) -> None:
    missing = sorted(META_COLUMNS - set(df.columns))
    if missing:
...
def run(args: argparse.Namespace) -> int:
    ...
    validate_required_columns(behavior_df, behavior_path)
